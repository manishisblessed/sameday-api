 ...
 * On success, sets an httpOnly cookie the proxy uses to authorize payout API calls.
 */
export async function POST(req: NextRequest) {
  ...
  const result = verifyGatePassword("settlement", body.password);
  ...
  res.cookies.set(gateCookieName("settlement"), result.token, gateCookieOptions());
  return res;
}
